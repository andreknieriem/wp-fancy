...
(function($) {
	$(document).ready(function() {
		...
		(function() {
			...
			if($('.helper-tool-data-attr').length > 0 && $dataAttrInput.val() !=''){
				var imageW = $(this).width();
				var imageH = $(this).height();
				
				//Calculate FocusPoint coordinates
				var focusX = $('.helper-tool-img').data('focus-x');
				var focusY = $('.helper-tool-img').data('focus-y');
				focusPointAttr.x = focusX;
				focusPointAttr.y = focusY;
				var offsetX = ((focusX / 2) + 0.5) * imageW;
				var offsetY = ((focusY / -2) + 0.5) * imageH;
				//Write values to input
				//printDataAttr();
				
				console.log(focusPointAttr.y);
